...
try:
    ...
    GSQTBJoin = ZDSJK+"\\dataset\\GSQTBJoin"
    Statistics = ZDSJK+"\\Statistics"
    topo_dataset_path = ZDSJK+"\\dataset"
    arcpy.env.workspace = ZDSJK+"\\dataset"

    def StatisticsFun():
        arcpy.AddField_management(GSQTB,"GSCOUNT","TEXT",20,"","","","NULLABLE")
        arcpy.Statistics_analysis(MMTB, Statistics, [["GSQTBH", "COUNT"]], "GSQTBH")
        arcpy.JoinField_management(GSQTB, "TBH", Statistics, "GSQTBH")
        arcpy.CalculateField_management(GSQTB, "GSCOUNT","!COUNT_GSQT!","PYTHON_9.3")
        arcpy.DeleteField_management(GSQTB, ["COUNT_GSQT", "FREQUENCY", "GSQTBH"])

    def checkGsqWithinGs():
        arcpy.SpatialJoin_analysis(GSQTB, MMTB, GSQTBJoin, "#", "#", "#")
        with arcpy.da.SearchCursor(GSQTBJoin, ("GSQBH", "Join_Count","GSZS","GSCOUNT")) as cursor:
            for row in cursor:
                if row[1] != row[2] or row[1] != int(row[3])  or row[2] != int(row[3]):
                    arcpy.AddError("古树群"+str(row[0])+"图形范围内"+str(row[1])+"株、属性表中所填古树株数"+str(row[2])+"株、每木调查表中汇总"+str(row[3])+"株三者不一致，请核实。")
                elif row[1] < 10:
                    arcpy.AddError("古树群"+str(row[0])+"中只有"+str(row[1])+"株古树，请核实。")
        arcpy.DeleteField_management(GSQTB, ["GSCOUNT"])
    ...
    def validateJd(jdStr):
        ...
        return re.match(reStr,str(jdStr))
    ...
    def deleteTemp():
        if arcpy.Exists(ZDSJK+"\\dataset\\checkZCDGsq"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\checkZCDGsq")
        if arcpy.Exists(ZDSJK+"\\dataset\\checkZCDgsmm"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\checkZCDgsmm")
        if arcpy.Exists(ZDSJK+"\\dataset\\checkZXJ"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\checkZXJ")
        if arcpy.Exists(ZDSJK+"\\Statistics"):
            arcpy.Delete_management(ZDSJK+"\\Statistics")
        featuresList = arcpy.ListFeatureClasses()
        for deatureClass in featuresList:
            arcpy.Delete_management(deatureClass)

    def deleteTempTopo():
        if arcpy.Exists(ZDSJK+"\\dataset\\gsmm_error_line"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\gsmm_error_line")
        if arcpy.Exists(ZDSJK+"\\dataset\\gsmm_error_point"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\gsmm_error_point")
        if arcpy.Exists(ZDSJK+"\\dataset\\gsmm_error_poly"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\gsmm_error_poly")
        if arcpy.Exists(ZDSJK+"\\dataset\\my_topo_error_line"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\my_topo_error_line")
        if arcpy.Exists(ZDSJK+"\\dataset\\my_topo_error_point"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\my_topo_error_point")
        if arcpy.Exists(ZDSJK+"\\dataset\\my_topo_error_poly"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\my_topo_error_poly")
        if arcpy.Exists(ZDSJK+"\\dataset\\joinDataCopyGsmm"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\joinDataCopyGsmm")
        if arcpy.Exists(ZDSJK+"\\dataset\\joinDataCopyGsq"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\joinDataCopyGsq")
        if arcpy.Exists(ZDSJK+"\\dataset\\landuse_singlepart"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\landuse_singlepart")
        if arcpy.Exists(ZDSJK+"\\dataset\\zxj_error_line"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\zxj_error_line")
        if arcpy.Exists(ZDSJK+"\\dataset\\zxj_error_point"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\zxj_error_point")
        if arcpy.Exists(ZDSJK+"\\dataset\\zxj_error_poly"):
            arcpy.Delete_management(ZDSJK+"\\dataset\\zxj_error_poly")
    ...
    def checkTb(fieldList,table):
        ...
        for field in gszd:
            ...
            if (field[0] not in fieldObj or field[1] != fieldObj[field[0]]):
                ...
                isCheckTB = False
        for key in fieldObj:
            if key not in gszdArr and key.lower() not in ['shape_leng', 'objectid', 'fid', 'shape_area', 'shape']:
    ...
    def SpatialJoin(joinData,table):
        ...
        arcpy.AddField_management(joinData,"FIDCOPY","SHORT",4,"","","","NULLABLE")
        arcpy.CalculateField_management(joinData, "FIDCOPY","!FID!","PYTHON_9.3")
        arcpy.DeleteField_management(joinData, ["OBJECTID"])
        arcpy.CopyFeatures_management(joinData, joinDataStr)
        arcpy.DeleteField_management(joinData, ["FIDCOPY"])
    ...
    def CheckZCD():
        if GSQTB != "":
            ...
            arcpy.CreateTopology_management(topo_dataset_path, "checkZCDGsq")
            arcpy.AddFeatureClassToTopology_management(
                topo_dataset_path + "\\" + "checkZCDGsq", checkData, 1, 1)
            arcpy.AddRuleToTopology_management(
                topo_dataset_path + "\\" + "checkZCDGsq", "Must Not Overlap (Area)", checkData, "", "", "")
            arcpy.ValidateTopology_management(
                topo_dataset_path + "\\" + "checkZCDGsq")
            mytopo = topo_dataset_path + "\\" + "checkZCDGsq"
    ...
    def checkGsqData():
        ...
        for row in gsqCursor:
            arcpy.SetProgressorLabel("检查古树群FID： " + str(row.getValue("FIDCOPY")))
            #---------------图斑号检查----------------
            ...
            if (tbh.strip() == ''):
                ...
                tbhBtBZstr += str(row.getValue("FIDCOPY")) + ","
                if (row.getValue("XIAN") + tbh.strip()) not in XBHrepeat:
            ...
            if (zysz.strip() == ''):
                ...
                zyszBtBZ += str(row.getValue("FIDCOPY")) + ","
                zyszArr = re.split("；|;",zysz)
                if len(zyszArr) > 3:
                    arcpy.AddMessage(zysz)
                    updateDataBySql("update GSQBZJG  set bz = '" +
                                    zyszBtBZ + "' where ZDMC = 'ZYSZ'")
                else:
                    for szdm in zyszArr:
                        bhdm = getDataBySql(
                            "select BHDM FROM SZB WHERE (BHDM = '" + szdm + "' OR ZGZZWM = '" + szdm + "') and SZJB = 3")
                        if len(bhdm) == 0:
                            zwzbhdm = getDataBySql("select ZWM FROM ZGZWZ WHERE ZWM = '" + szdm + "'")
                            if len(zwzbhdm) == 0:
                                updateDataBySql("update GSQBZJG  set bz = '" +
                                                zyszBtBZ + "' where ZDMC = 'ZYSZ'")
            #--------------面积----------------
            ...
            if (str(gszs).strip() == '' or gszs == 0):
                ...
                gszsBtBZ += str(row.getValue("FIDCOPY")) + ","
                if gszs < 10:
                    updateDataBySql("update GSQBZJG  set bz = '" +
            ...
            if (str(lfpjg).strip() == '' or lfpjg == 0):
                ...
                lfpjgBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(lfpjxj).strip() == '' or lfpjxj == 0):
                ...
                lfpjxjBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(pjsl).strip() == '' or pjsl == 0):
                ...
                pjslBtBZ += str(row.getValue("FIDCOPY")) + ","
                if pjsl < 100:
                    updateDataBySql("update GSQBZJG  set bz = '" +
            ...
            if (str(hb).strip() == '' or hb == 0):
                ...
                hbBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(pd).strip() == '' or pd == 0):
                ...
                pdBtBZ += str(row.getValue("FIDCOPY")) + ","
                if pd > 6 or pd < 1:
                    updateDataBySql("update GSQBZJG  set bz = '" +
            ...
            if (str(px).strip() == '' or px == 0):
                ...
                pxBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(trlx).strip() == '' or trlx == 0):
                ...
                trlxBtBZ += str(row.getValue("FIDCOPY")) + ","
                trlxdm = getDataBySql(
            ...
            if (str(tchd).strip() == '' or tchd == 0):
                ...
                tchdBtBZ += str(row.getValue("FIDCOPY")) + ","
                if tchd > 200:
                    updateDataBySql("update GSQBZJG  set bz = '" +
            ...
            xmzlBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            xmmdBtBZ += str(row.getValue("FIDCOPY")) + ","
            if xmmd > 2000:
                updateDataBySql("update GSQBZJG  set bz = '" +
            ...
            dbwzlBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            dbwmdBtBZ += str(row.getValue("FIDCOPY")) + ","
            if dbwmd > 100:
                updateDataBySql("update GSQBZJG  set bz = '" +
            ...
            if (mdbhsz.strip() == ''):
                ...
                mdbhszBtBZ += str(row.getValue("FIDCOPY")) + ","
                mdbhszdm = getDataBySql("select BHDM FROM SZB WHERE (BHDM = '" +
                                        mdbhsz + "' or ZGZZWM = '" + mdbhsz + "')  and SZJB = 3")
                if len(mdbhszdm) == 0:
                    zwzmdbhsz = getDataBySql("select ZWM FROM ZGZWZ WHERE ZWM = '" + mdbhsz + "'")
                    if len(zwzmdbhsz) == 0:
                        updateDataBySql("update GSQBZJG  set bz = '" +
            ...
            if (mdszk.strip() == ''):
                ...
                mdbSZKBtBZ += str(row.getValue("FIDCOPY")) + ","
                if mdbhsz.strip() == '':
                    updateDataBySql("update GSQBZJG  set bz = '" +
                                    mdbSZKBtBZ + "' where ZDMC = 'MDSZK'")
                else:
                    mdbhszK = getDataBySql("select K FROM SZB WHERE (BHDM = '" +
                                           mdbhsz + "' or ZGZZWM = '" + mdbhsz + "')  and SZJB = 3 UNION select KM as K from ZGZWZ WHERE ZWM = '"+mdbhsz+"'")
                    if len(mdbhszK) == 0:
                        updateDataBySql("update GSQBZJG  set bz = '" +
                                        mdbSZKBtBZ + "' where ZDMC = 'MDSZK'")
            ...
            if (MDSZS.strip() == ''):
                ...
                MDSZSBtBZ += str(row.getValue("FIDCOPY")) + ","
                if mdbhsz.strip() == '':
                    updateDataBySql("update GSQBZJG  set bz = '" +
                                    MDSZSBtBZ + "' where ZDMC = 'MDSZS'")
                else:
                    mdbhszS = getDataBySql("select S FROM SZB WHERE (BHDM = '" +
                                           mdbhsz + "' or ZGZZWM = '" + mdbhsz + "')  and SZJB = 3 UNION select SM as S from ZGZWZ WHERE ZWM = '"+mdbhsz+"'")
                    if len(mdbhszS) == 0:
                        updateDataBySql("update GSQBZJG  set bz = '" +
                                        MDSZSBtBZ + "' where ZDMC = 'MDSZS'")
                    elif len(mdbhszS) > 0 and mdbhszS[0][0] != MDSZS:
                        updateDataBySql("update GSQBZJG  set bz = '" +
                                        MDSZSBtBZ + "' where ZDMC = 'MDSZS'")
            #------------照片名---------------
            ...
            if (str(zpm).strip() == ''):
                ...
                zpmBtBZ += str(row.getValue("FIDCOPY")) + ","
                zpmArr = re.split("；|;|,|，|、",str(zpm))
                for zpmStr in zpmArr:
                    writeZpmExcel(OutFolder+"\\gsqzpm.xls",zpmStr)
                    if zpmStr[0:10] != row.getValue("GSQBH") :
                        updateDataBySql("update GSQBZJG  set bz = '" +
            ...
            if (str(dcrq).strip() == ''  or dcrq =='None' or str(dcrq)[0:4] == '1899'):
                if dcrqisBT == "是":
                    updateDataBySql("update GSQBZJG SET CWSL = " +
                ...
                dcrqBtBZ += str(row.getValue("FIDCOPY")) + ","
                if not is_valid_date(str(dcrq)):
                    updateDataBySql("update GSQBZJG  set bz = '" +
                                    dcrqBtBZ + "' where ZDMC = 'DCRQ'")
                elif int(str(dcrq)[0:4]) > 2017:
                    updateDataBySql("update GSQBZJG  set bz = '" +
                                    dcrqBtBZ + "' where ZDMC = 'DCRQ'")
            #------------审核人---------------
            ...
            if (str(shrq).strip() == '' or shrq =='None'  or str(shrq)[0:4] == 1899):
                if shrqisBT == "是":
                    updateDataBySql("update GSQBZJG SET CWSL = " +
                ...
                shrqBtBZ += str(row.getValue("FIDCOPY")) + ","
                if not is_valid_date(str(shrq)):
                    updateDataBySql("update GSQBZJG  set bz = '" +
                                    shrqBtBZ + "' where ZDMC = 'SHRQ'")
                elif  int(str(shrq)[0:4]) > 2017:
                    updateDataBySql("update GSQBZJG  set bz = '" +
                                    shrqBtBZ + "' where ZDMC = 'SHRQ'")
            #------------古树群编号---------------
            ...
            if (gsqbh.strip() == ''):
                ...
                if NewGsqBh != gsqbh:
                    ...
                    gsqbhBtBZ += str(row.getValue("FIDCOPY")) + ","
                    updateDataBySql("update GSQBZJG  set bz = '" +
                                    gsqbhBtBZ + "' where ZDMC = 'GSQBH'")
            gsqCursor.updateRow(row)
    ...
    def checkMmData():
        ...
        for row in mmCursor:
            arcpy.SetProgressorLabel("检查每木FID： " + str(row.getValue("FIDCOPY")))
            #-------------------调查顺序号--------------------
            ...
            if (dcbh.strip() == ''):
                ...
                dcbhBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (gsmmbh.strip() == ''):
                ...
                if NewGsmmBh != gsmmbh:
                    ...
                    gsmmbhBtBZ += str(row.getValue("FIDCOPY")) + ","
                    updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (ldm.strip() == ''):
                ...
                ldmBtBZ += str(row.getValue("FIDCOPY")) + ","
                if zwm.strip() == '':
                    updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (ke.strip() == ''):
                ...
                keBtBZ += str(row.getValue("FIDCOPY")) + ","
                if zwm.strip() == '':
                    updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (shu.strip() == ''):
                ...
                shuBtBZ += str(row.getValue("FIDCOPY")) + ","
                if shu.strip() == '':
                    updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (str(szcs).strip() == '' or szcs == 0):
                ...
                szcsBtBZ += str(row.getValue("FIDCOPY")) + ","
                if szcs not in [1, 2]:
                    updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (str(fbtd).strip() == '' or fbtd == 0):
                ...
                fbtdBtBZ += str(row.getValue("FIDCOPY")) + ","
                if fbtd not in [1, 2]:
                    updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (str(jd).strip() == '' or jd == 0):
                ...
                jdBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(WD).strip() == '' or WD == 0):
                ...
                WDBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(qs).strip() == '' or qs == 0):
                ...
                qsBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(bsdm).strip() == ''):
                ...
                bsdmBtBZ += str(row.getValue("FIDCOPY")) + ","
                if bsdm not in ['1', '2', '3', '4', '5', '6', '7'] or (bsdm in ['2','3','6','7'] and (row.getValue("GSLS1").strip() == "" and row.getValue("GSLS2").strip() == ""  and row.getValue("GSLS3").strip() == "" )):
                    arcpy.AddMessage(str(row.getValue("FIDCOPY"))+":"+bsdm)
                    updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (zldm.strip() == ''):
                ...
                zldmBtBZ += str(row.getValue("FIDCOPY")) + ","
                zldmMC = getDataBySql("select ZGZZWM FROM SZB WHERE BHDM = '" +
                                      zldm + "' AND SZJB = 3 UNION select ZWM AS ZGZZWM from ZGZWZ WHERE ZWM = '"+zldm+"'")
                if zldm != '9999':
            ...
            if (str(zssl).strip() == '' or zssl == 0):
                ...
                zsslBtBZ += str(row.getValue("FIDCOPY")) + ","
                if (row.getValue("BSDM") in ['2'] and zssl!=0) or zssl > 1500 or (row.getValue("BSDM") in ['1', '3', '5', '7'] and zssl < 100) or (row.getValue("GSDJ")== 1 and zssl < 500) or (row.getValue("GSDJ")==2 and (zssl < 300 or zssl >= 500)) or (row.getValue("GSDJ")==3 and (zssl < 100 or zssl >= 300)):
                    updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (str(gcsl).strip() == '' or gcsl == 0):
                ...
                gcslBtBZ += str(row.getValue("FIDCOPY")) + ","
                if (row.getValue("ZSSL") != 0):
                    updateDataBySql("update MMBZJG  set bz = '" +
                                    gcslBtBZ + "' where ZDMC = 'GCSL'")
                elif ((row.getValue("BSDM") in ['2'] and gcsl!=0) or (row.getValue("BSDM") in ['1', '3', '5', '7'] and gcsl < 100) or gcsl > 1500 or (row.getValue("GSDJ")==1 and gcsl < 500) or (row.getValue("GSDJ")==2 and (gcsl < 300 or gcsl >= 500)) or (row.getValue("GSDJ")==3 and (gcsl < 100 or gcsl >= 300))):
                    updateDataBySql("update MMBZJG  set bz = '" +
                                    gcslBtBZ + "' where ZDMC = 'GCSL'")
            #---------------------胸径------------------
            xj=row.getValue("XJ")
            ...
            if (str(xj).strip() == '' or xj == 0):
                ...
                xjBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(gsdj).strip() == ''):
                ...
                gsdjBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(zzsj).strip() == '' or str(zzsj).strip() == 'None' or str(zzsj)[0:4] == '1899'):
                if zzsjisBT == "是":
                ...
                zzsjBtBZ += str(row.getValue("FIDCOPY")) + ","
                if int(str(zzsj)[0:4]) > 2017 or not is_valid_date(str(zzsj)) or (row.getValue("BSDM") in ['2', '3','6', '7'] and str(zzsj)[0:4] == '1899') or (row.getValue("BSDM") in ['1','4'] and str(zzsj)[0:4] != '1899') :
                    updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (str(zzr).strip() == ''):
                ...
                zzrBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(sg).strip() == '' or sg == 0):
                ...
                sgBtBZ += str(row.getValue("FIDCOPY")) + ","
                if sg > 50:
                    updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (str(gfdx).strip() == '' or gfdx == 0):
                ...
                gfdxBtBZ += str(row.getValue("FIDCOPY")) + ","
                if gfdx > 40:
                    updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (str(GFNB).strip() == '' or GFNB == 0):
                ...
                GFNBBtBZ += str(row.getValue("FIDCOPY")) + ","
                if GFNB > 40:
                    updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if row.getValue("GFNB") == 0 or row.getValue("GFDX") == 0:
                ...
                GFPJBtBZ += str(row.getValue("FIDCOPY")) + ","
                updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (str(PW).strip() == '' or PW == 0):
                ...
                PWBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(TRJMD).strip() == '' or TRJMD == 0):
                ...
                TRJMDBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(SZS).strip() == '' or SZS == 0):
                ...
                SZSBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(SZHJ).strip() == '' or SZHJ == 0):
                ...
                SZHJBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(YXSZHJYS).strip() == '' or YXSZHJYS == 0):
                ...
                YXSZHJYSBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            SHQKBtBZ += str(row.getValue("FIDCOPY")) + ","
            if (SHQK not in [1,2,3,4] and row.getValue("SZS") in [2,3]):
                updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (str(XZYY).strip() == '' or XZYY == 0):
                ...
                XZYYBtBZ += str(row.getValue("FIDCOPY")) + ","
                if XZYY not in [1,2,3]:
                    updateDataBySql("update MMBZJG  set bz = '" +
            ...
            if (str(SZJDJZ).strip() == '' or SZJDJZ == 0):
                ...
                SZJDJZBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(GSLS1).strip() == '' or GSLS1 == 0):
                ...
                GSLS1BtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(GSLS2).strip() == '' or GSLS2 == 0):
                ...
                GSLS2BtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(GSLS3).strip() == '' or GSLS3 == 0):
                ...
                GSLS3BtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(BHXZ).strip() == '' or BHXZ == 0):
                ...
                BHXZBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(YHFZXZ).strip() == '' or YHFZXZ == 0):
                ...
                YHFZXZBtBZ += str(row.getValue("FIDCOPY")) + ","
            ...
            if (str(zpm).strip() == ''):
                ...
                zpmArr = re.split("；|;|,|，|、",str(zpm))
                for zpmStr in zpmArr:
                    writeZpmExcel(OutFolder+"\\gsmmzpm.xls",zpmStr)
                    if zpmStr[0:11] != row.getValue("GDMMBH") :
                        updateDataBySql("update MMBZJG  set bz = '" +
                                        zpmBtBZ + "' where ZDMC = 'ZPM'")
            #------------有无标本---------------
            ...
            if (str(YWBB).strip() == '' or YWBB == 0):
                ...
                YWBBBtBZ += str(row.getValue("FIDCOPY")) + ","
                if YWBB not in [1,2]:
                    updateDataBySql("update MMBZJG  set bz = '" +
    ...
    def exportResult():
        ...
        if GSQTB != "":
            arcpy.DeleteField_management(outFildGsq, ["FIDCOPY"])
            arcpy.CopyFeatures_management(outFildGsq, OutFolder+"\\古树群调查.shp")
            arcpy.DeleteField_management(OutFolder+"\\古树群调查.shp", ["Geometry_L","Geometry_A"])
        arcpy.DeleteField_management(outFildGsmm, ["FIDCOPY"])
        arcpy.CopyFeatures_management(outFildGsmm, OutFolder+"\\古树大树名木每木调查.shp")
        arcpy.DeleteField_management(OutFolder+"\\古树大树名木每木调查.shp", ["Geometry_L","Geometry_A"])
    ...
    def addExcel():
        ...
        sheet.write(0,3,'必填项检查错误数',style0)
        sheet.write(0,4,'错误数据FID',style0)
        col0=sheet.col(0)
        col1=sheet.col(1)
        col2=sheet.col(2)
        col3=sheet.col(3)
    ...
    def addZpmExcel(fileName):
        ...
        sheet.write(0,0,'mingzi',style0)
        col0=sheet.col(0)
        col0.width=256*20
        workbook.save(outPath)
    def writeZpmExcel(fileName,col1):
        outPath = fileName
    ...
    addExcel()
    addZpmExcel(OutFolder+"\\gsqzpm.xls")
    addZpmExcel(OutFolder+"\\gsmmzpm.xls")
    initData()#清空错误缓存
    deleteTemp()
    StatisticsFun()
    checkGsqWithinGs()
    ...
    if isCheckTB:#通过结构检查  才能进行数据质检
        ...
        if GSQTB != "":
            ...
            arcpy.SetProgressor("step", "古树群数据检查中，喝杯茶歇歇吧...",
                        0, int(str(arcpy.GetCount_management(GSQTB))), 1)
            checkGsqData()#古树群逻辑检查
            arcpy.ResetProgressor()
        arcpy.SetProgressor("step", "每木数据检查中，喝杯茶歇歇吧...",
                    0, int(str(arcpy.GetCount_management(MMTB))), 1)
        checkMmData()#古树名木逻辑检查
        arcpy.ResetProgressor()
        exportResult()#导出成果
        exportError('GSQBZJG')
        exportError('MMBZJG')
        deleteTempTopo()#删除缓存数据
